...
```sh
claude --plugin-dir ./integrations/claude-code/voidhash
```

Open `/mcp`, approve the `voidhash` server if prompted, and choose **Authenticate**. You can also run `claude mcp login voidhash`. Complete sign-in and organization selection in the browser.
