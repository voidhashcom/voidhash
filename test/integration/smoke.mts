...
const AgentFrame = Schema.Struct({
  ...
  event: Schema.optional(Schema.NullOr(Schema.Struct({ type: Schema.optional(Schema.Unknown) }))),
});
...
const authenticateSocket = (
  ...
  Effect.callback<ReadonlyArray<typeof SocketMessage.Type>, SmokeError>((resume) => {
    ...
    Effect.timeoutOrElse({
      duration: "10 seconds",
      orElse: () =>
        Effect.fail(new SmokeError({ message: "Timed out waiting for the WebSocket transaction" })),
    }),
  );
...
const smokeAgentSession = (url: string) =>
  Effect.gen(function* () {
    ...
    const organizationId = (yield* envString("SELFHOST_AGENT_SMOKE_ORGANIZATION_ID", "")).trim();
...
const program = Effect.gen(function* () {
  ...
  const database = yield* attempt("Create database", () =>
    sdk.createDatabase({
      ...
      description: "Node integration smoke",
    }),
  );
  ...
  const cleanup = Effect.gen(function* () {
    ...
    if (createdCollectionId) {
      ...
      yield* attempt("Delete collection", () => database.deleteCollection(createdCollectionId));
    }
