  ...
  Effect.gen(function* () {
    ...
    if (forbidden.length > 0) {
      return yield* new ForbiddenPackagesError({
        message: `Node runtime contains Cloudflare-only packages:\n${forbidden
          .map((pkg) => `- ${pkg.name}@${pkg.version}`)
          .join("\n")}`,
      });
    }
    yield* Console.log(`Node runtime boundary OK — ${installed.size} installed packages checked.`);
  });
...
const deployAndInspect = Effect.gen(function* () {
  ...
  const deploymentRoot = yield* fs.makeTempDirectoryScoped({
    prefix: "voidhash-node-runtime-boundary-",
  });
