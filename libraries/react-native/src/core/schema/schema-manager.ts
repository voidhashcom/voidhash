...
const toFetchError = (cause: unknown): FailedToFetchSchemaError => {
  ...
  return new FailedToFetchSchemaError(
    ...
    make: Effect.gen(function* () {
      ...
      const scheduleBackgroundRefresh = (
        ...
        fetchFromServer(distinctId).pipe(
          ...
          Effect.catch((cause) =>
            Effect.logDebug(
              "[voidhash] schema background refresh failed",
              { cause }
            )
          ),
      ...
      const resolveSchema = ({
        ...
        Effect.gen(function* () {
          ...
          if (!appVersion) {
            ...
            const schema = yield* fetchFromServer(distinctId).pipe(
              Effect.mapError(toFetchError)
            );
            publishSchema(schema);
            return schema;
          }
