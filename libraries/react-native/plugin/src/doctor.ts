...
import { validateVoidhashExpoPluginOptions } from "./withVoidhashReactNative";
...
export const diagnoseVoidhashIntegration = (snapshot: DoctorProjectSnapshot): DoctorReport => {
  const findings: DoctorFinding[] = [];
  try {
    validateVoidhashExpoPluginOptions(snapshot.options);
  } catch (error) {
  ...
  if (
    ios?.skAdNetworkPostbackEndpoint &&
  ...
  if (
    ios?.adAttributionKitPostbackEndpoint &&
