...
const withMotion = <Props extends MotionVisualProps & { style?: MotionStyleProp }>(
  props: Props,
) => {
...
const TreeView = forwardRef<MotionNodeHandle, ViewProps>(
  ({ style, children, ...props }, _ref): ReactNode =>
    createElement(TREE_ELEMENT_TYPES.view, { style, ...withMotion({ ...props, style }) }, children),
);

const TreeText = forwardRef<MotionNodeHandle, TextProps>(
  ({ style, children, ...props }, _ref): ReactNode =>
...
const TreePressable = forwardRef<MotionNodeHandle, PressableHostProps>(
  ({ style, children, actionName, onPress, ...props }, _ref): ReactNode =>
    createElement(
...
const TreeScrollView = forwardRef<ScrollViewHandle, ScrollViewProps>(
  ({ style, contentContainerStyle, horizontal, children, ...props }, _ref): ReactNode =>
    createElement(
...
const TreeImage = forwardRef<MotionNodeHandle, ImageProps>(
  ({ style, source, resizeMode, ...props }, _ref): ReactNode =>
