...
const config: S3ObjectStoreConfig = {
  bucketName: process.env.PLATFORM_NODE_S3_BUCKET ?? "voidhash-public",
  region: process.env.PLATFORM_NODE_S3_REGION ?? "us-east-1",
  endpoint: process.env.PLATFORM_NODE_S3_ENDPOINT ?? "http://127.0.0.1:9000",
  accessKeyId: process.env.PLATFORM_NODE_S3_ACCESS_KEY_ID ?? "voidhash",
  secretAccessKey: Redacted.make(
    process.env.PLATFORM_NODE_S3_SECRET_ACCESS_KEY ?? "password",
  ),
  forcePathStyle: true,
};

const storeLayer = (input: S3ObjectStoreConfig = config) =>
  Layer.merge(S3ObjectStoreLive(input), NodePlatformRuntimeLive);
const describeS3 = process.env.PLATFORM_NODE_S3_TEST === "1" ? describe : describe.skip;

describeS3("S3-compatible object store", () => {
