...
const config: SmtpMailerConfig = {
  host: process.env.PLATFORM_NODE_SMTP_HOST ?? "127.0.0.1",
  port: Number(process.env.PLATFORM_NODE_SMTP_PORT ?? "1025"),
  defaultFrom: { address: "noreply@voidhash.local", name: "Voidhash" },
  verifyOnStart: true,
};

const mailerLayer = (input: SmtpMailerConfig = config) =>
  Layer.merge(SmtpMailerLive(input), NodePlatformRuntimeLive);
const describeSmtp = process.env.PLATFORM_NODE_SMTP_TEST === "1" ? describe : describe.skip;
const mailpitApi = process.env.PLATFORM_NODE_MAILPIT_API ?? "http://127.0.0.1:8025";
