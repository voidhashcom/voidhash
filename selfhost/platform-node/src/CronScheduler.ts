...
const ensureTable = (sql: SqlClient.SqlClient) =>
  sql.withTransaction(
    Effect.gen(function* () {
      yield* sql`SELECT pg_advisory_xact_lock(hashtext('voidhash_platform_cron_schema_v1'))`;
      yield* sql`
                ...
                Effect.matchCauseEffect({
                  onFailure: (cause) =>
                    releaseRun(sql, job.name, claimed.leaseToken, Date.now()).pipe(
                      ...
                      Effect.andThen(
                        Effect.fail(
                          schedulerError(job.name, "run", Cause.pretty(cause)),
                        ),
                      ),
                    ),
                  onSuccess: () =>
...
export const PgCronSchedulerLive = (
  config: PgPlatformConfig,
): Layer.Layer<CronScheduler> =>
  Layer.effect(
    CronScheduler,
    Effect.gen(function* () {
