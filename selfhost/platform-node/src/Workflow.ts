    ...
    Exit.match(result.exit, {
      onFailure: (cause) => {
        ...
        return {
          ...
          error:
            error instanceof WorkflowRunnerError
              ? error
              : runnerError("unknown", "poll", error),
        };
      },
      ...
      return catchRunnerCause(
        engine.register(native, (payload, executionId) => {
          const context: WorkflowHandlerContext = {
            ...
            sleepUntil: (name, scheduledTime) =>
              sleepUntil(workflow.name, name, scheduledTime),
          };
          return catchRunnerCause(
...
export const PgWorkflowRunnerLive = (
  config: PgPlatformConfig,
): Layer.Layer<WorkflowRunner> =>
  Layer.effect(
    WorkflowRunner,
    Effect.map(WorkflowEngine.WorkflowEngine, makeRunner),
  ).pipe(Layer.provide(PgWorkflowEngineLive(config)));
