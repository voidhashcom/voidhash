 ...
 *
 * Opt in with `PLATFORM_CLUSTER_PG_TEST=1` and a reachable database, matching
 * how the single-node adapter suites gate.
 */
const enabled = process.env.PLATFORM_CLUSTER_PG_TEST === "1";

const SqlLive = PgClient.layer({
  host: process.env.PLATFORM_CLUSTER_PG_HOST ?? "127.0.0.1",
  port: Number(process.env.PLATFORM_CLUSTER_PG_PORT ?? "5432"),
  database: process.env.PLATFORM_CLUSTER_PG_DATABASE ?? "voidhash",
  username: process.env.PLATFORM_CLUSTER_PG_USERNAME ?? "voidhash",
  password: Redacted.make(process.env.PLATFORM_CLUSTER_PG_PASSWORD ?? "password"),
});
...
if (enabled) {
  ...
  durableEntityHostConformance({ name: "cluster/postgres", layer: entityLayer });
  ...
  describe.skip("cluster/postgres", () => {
    it("requires PLATFORM_CLUSTER_PG_TEST=1", () => {});
  });
}
