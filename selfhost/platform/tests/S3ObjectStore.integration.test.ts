...
const readConfig = Effect.gen(function* () {
  const config: S3ObjectStoreConfig = {
    bucketName: yield* Config.string("PLATFORM_SELFHOST_S3_BUCKET").pipe(
      Config.withDefault("voidhash-public"),
    ),
    region: yield* Config.string("PLATFORM_SELFHOST_S3_REGION").pipe(
      Config.withDefault("us-east-1"),
    ),
    endpoint: yield* Config.string("PLATFORM_SELFHOST_S3_ENDPOINT").pipe(
      Config.withDefault("http://127.0.0.1:9000"),
    ),
    accessKeyId: yield* Config.string("PLATFORM_SELFHOST_S3_ACCESS_KEY_ID").pipe(
      Config.withDefault("voidhash"),
    ),
    secretAccessKey: yield* Config.redacted("PLATFORM_SELFHOST_S3_SECRET_ACCESS_KEY").pipe(
...
const storeLayer = (
  ...
  Layer.unwrap(
    readConfig.pipe(
      Effect.map((config) =>
        Layer.merge(S3ObjectStoreLive(adjust(config)), SelfhostPlatformRuntimeLive),
      ),
    ),
  );
    ...
    Effect.runPromise(
      Effect.gen(function* () {
        ...
        const error = yield* Effect.gen(function* () {
          ...
          Effect.provide(storeLayer((input) => ({ ...input, bucketName: missingBucket }))),
        );
