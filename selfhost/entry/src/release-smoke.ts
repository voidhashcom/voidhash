...
import { BackendSnapshotHtmlRendererLive } from "@voidhash/backend/src/PaywallSnapshotHtmlRenderer.ts";
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const hostContext = yield* Layer.build(
        makeMimicNodeHostLive(getMimicNodeConfig()),
      );
      ...
      const infrastructure = makeBackendInfrastructureLive(config, workos).pipe(
