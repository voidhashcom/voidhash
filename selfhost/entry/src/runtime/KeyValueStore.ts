...
const ensureTable = (sql: SqlClient.SqlClient) =>
  sql.withTransaction(
    Effect.gen(function* () {
      yield* sql`SELECT pg_advisory_xact_lock(hashtext('orbian_kv_schema_v1'))`;
      yield* sql`
...
const expiry = (now: number, options: KeyValuePutOptions | undefined): number | null => {
  ...
  return typeof ttl === "number" && Number.isFinite(ttl) && ttl > 0 ? now + Math.floor(ttl) : null;
};
...
const makeStore = (sql: SqlClient.SqlClient): KeyValueStoreShape => ({
  ...
  delete: (namespace, key) =>
    PlatformRuntime.pipe(
      Effect.andThen(
        sql`DELETE FROM platform_key_value WHERE namespace = ${namespace} AND key = ${key}`,
      ),
      Effect.asVoid,
...
export const PgKeyValueStoreLive = (config: PgPlatformConfig): Layer.Layer<KeyValueStore> =>
  Layer.effect(
    KeyValueStore,
    Effect.gen(function* () {
