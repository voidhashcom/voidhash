...
const jsonCodec = (schema: Schema.Top): JsonCodec =>
  Schema.toCodecJson(schema) as unknown as JsonCodec;
...
const ensureTables = (sql: SqlClient.SqlClient) =>
  sql.withTransaction(
    Effect.gen(function* () {
      yield* sql`SELECT pg_advisory_xact_lock(hashtext('orbian_workflow_schema_v1'))`;
      yield* sql`
...
const workflowResultCodec = (workflow: Workflow.Any) =>
  ...
  ) as unknown as JsonCodec;

const encodePayload = (workflow: Workflow.Any, payload: object) =>
  Schema.encodeEffect(jsonCodec(workflow.payloadSchema))(payload).pipe(Effect.flatMap(encodeJson));
...
const encodeWorkflowResult = (workflow: Workflow.Any, result: Workflow.Result<unknown, unknown>) =>
...
const makePgEngine = (sql: SqlClient.SqlClient) =>
  Effect.gen(function* () {
      ...
      return pollResult(workflow, executionId).pipe(
        Effect.flatMap((result) =>
          Option.isSome(result)
            ...
            : Effect.sleep("20 millis").pipe(Effect.andThen(waitForResult(workflow, executionId))),
        ),
      );
    }
    ...
    const executeWorkflow = <const Discard extends boolean>(
      ...
      Effect.gen(function* () {
        yield* ensureExecution(workflow, options.executionId, options.payload, options.parent);
    ...
    engine = WorkflowEngine.makeUnsafe({
      ...
      deferredResult: (deferred) =>
        Effect.gen(function* () {
          ...
          `;
          return rows[0] ? Option.some(rows[0].exit as Exit.Exit<unknown, unknown>) : Option.none();
        }).pipe(Effect.orDie),
