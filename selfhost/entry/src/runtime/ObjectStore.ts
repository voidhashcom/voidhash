...
import { ObjectStore, ObjectStoreError, type ObjectStoreShape } from "@orbian/sdk/ObjectStore";
...
const storeError = (config: S3ObjectStoreConfig, key: string, operation: string, cause: unknown) =>
...
const makeStore = (config: S3ObjectStoreConfig, client: S3.Type): ObjectStoreShape => ({
  bucketName: config.bucketName,
    ...
    PlatformRuntime.pipe(
      ...
      Effect.catch((cause) =>
        isNotFound(cause)
          ? Effect.succeedNone
          : Effect.fail(
              cause instanceof ObjectStoreError ? cause : storeError(config, key, "get", cause),
            ),
      ),
    ),
  head: (key) =>
...
export const S3ObjectStoreLive = (config: S3ObjectStoreConfig): Layer.Layer<ObjectStore> =>
  Layer.effect(
    ObjectStore,
