...
describe("Testing decoding of signed data", () => {
  it("should decode an app transaction", async () => {
    ...
    const appTransaction = unwrapOptionsDeep(
      await Effect.runPromise(verifier.verifyAndDecodeAppTransaction(signedAppTransaction)),
    );
  ...
  it("should decode a renewal info", async () => {
    ...
    const renewalInfo = unwrapOptionsDeep(
      await Effect.runPromise(verifier.verifyAndDecodeRenewalInfo(signedRenewalInfo)),
    );
  ...
  it("should decode a transaction info", async () => {
    ...
    const transaction = unwrapOptionsDeep(
      await Effect.runPromise(verifier.verifyAndDecodeTransaction(signedTransaction)),
    );
  ...
  it("should decode a signed notification", async () => {
    ...
    const notification = unwrapOptionsDeep(
      await Effect.runPromise(verifier.verifyAndDecodeNotification(signedNotification)),
    );
  ...
  it("should detect sandbox env from external purchase token externalPurchaseId prefix", async () => {
    const signedNotification = createSignedDataFromJson(
