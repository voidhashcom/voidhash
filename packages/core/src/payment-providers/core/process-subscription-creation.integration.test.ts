...
// 		const transaction = await h.db.primary.query.transactions.findFirst({
// 			where: and(
...
// 		expect(transaction?.paymentProviderConfigurationProductId).toBe(
// 			paymentProviderConfigurationProduct.id
// 		);
// 		expect(transaction?.providerEnvironment).toBe("production");
// 		expect(transaction?.customerId).toBe(customerInsert.id);
// 		expect(transaction?.occurredAt).toBeDefined();
// 		expect(transaction?.storeTransactionId).toBeDefined();
// 		expect(transaction?.id).toBe(subscription!.initialTransactionId);
// 		expect(transaction?.id).toBe(subscription!.latestTransactionId);
...
// 		expect(customerUnlockedPerks[0]!.status).toBe("active");
