 ...
 * name is stored, shared by the PostgreSQL (community) and hosted editions.
 *
 ...
 *
 * The registry mirrors the shape of the internal feature flag registry: code
 * holds the entries and their defaults, the database holds explicit overrides,
 * and absence of an override means "use the default".
 */
import { constant, pick } from "@voidhash/lib/lang";

import { RESERVED_REVENUE_EVENT_NAMES } from "../internalAnalytics/InternalAnalyticsEvents.ts";

/** Which product edition an admission decision is being made for. */
export type AnalyticsEdition = "cloud" | "oss";
...
export interface BuiltinEventAdmissionEntry {
  ...
  readonly defaultEnabled: Readonly<Record<AnalyticsEdition, boolean>>;
...
const lifecycleEntry = (
  ...
  name: string,
  description: string,
  defaultEnabled: Readonly<Record<AnalyticsEdition, boolean>>,
): BuiltinEventAdmissionEntry => ({
  key: eventName,
  name,
  description,
...
export const BUILTIN_EVENT_ADMISSION_LIST: readonly BuiltinEventAdmissionEntry[] = constant([
  ...
  lifecycleEntry(
    "$app_updated",
    "App updated",
    "First launch after the app version changed.",
  ...
  },
  {
    ...
    defaultEnabled: { cloud: true, oss: false },
    warning:
      "Self-hosted installs store the event only; resolving it into a person profile is a cloud capability.",
  },
]);
...
const entriesByEventName: ReadonlyMap<string, BuiltinEventAdmissionEntry> = new Map(
  BUILTIN_EVENT_ADMISSION_LIST.flatMap((entry) =>
    entry.eventNames.map((eventName): [string, BuiltinEventAdmissionEntry] => [eventName, entry]),
  ),
);
...
export interface EventAdmissionPolicy {
  ...
  readonly builtinEventOverrides: Readonly<Record<string, boolean>>;
...
export const emptyEventAdmissionPolicy: EventAdmissionPolicy = constant({
...
export const isBuiltinEntryEnabled = ({
  ...
  readonly edition: AnalyticsEdition;
  readonly entry: BuiltinEventAdmissionEntry;
  readonly policy: EventAdmissionPolicy;
...
export const admitEvent = ({
  ...
  return pick(policy.customEventBlocklist.includes(eventName), {
    admitted: false,
