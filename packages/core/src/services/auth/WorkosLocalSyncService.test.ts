...
} from "./WorkosLocalSyncService.ts";
...
const makeTestLayer = (options: {
    ...
    Layer.succeed(
      Workos,
      {
        setUserExternalId: (workosUserId: string, externalId: string) => {
          externalIdCalls.push({ externalId, workosUserId });
          return options.setExternalIdFailure
            ? Effect.fail({ _tag: "WorkosError" } as never)
            : Effect.void;
        },
      } as never,
    ),
  ...
  return {
    ...
    layer: Layer.mergeAll(
      Dependencies,
      WorkosLocalSyncService.layer.pipe(Layer.provide(Dependencies)),
    ),
    membershipCalls,
  };
};

describe("WorkosLocalSyncService", () => {
  it("resolves the local user and delegates optional membership synchronization", async () => {
    const testLayer = makeTestLayer();
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        const sync = yield* WorkosLocalSyncService;
  ...
  it("maps membership extension failures to the authentication-facing error", async () => {
    ...
    if (result._tag === "Failure") {
      expect(result.failure).toBeInstanceOf(WorkosLocalSyncServiceError);
