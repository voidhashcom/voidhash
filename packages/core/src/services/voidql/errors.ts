 ...
 * each `_tag` via `Effect.catchTags` into its `Rpc/`-prefixed counterpart (the
 * established 3-layer convention). Messages are path-precise but carry **no
 * ClickHouse internals** — client-facing surfaces are uniform opaque envelopes to
 * starve timing / row-count side-channels (§7 L7, §13).
 */
...
/**
 * The currency of the agent repair loop and the Monaco red-underline: a typed,
 * span-precise compile diagnostic. Returned as *data* (not thrown) by
 * `validateQuery`, and mirrored by the thrown error's fields on the run path.
 */
 ...
 * query is rejected (never null-substituted) so a `WHERE email = …` cannot act as
 * a row-count oracle (§9).
 */
 ...
 * to clients as an opaque execution error (§10, §20).
 */
