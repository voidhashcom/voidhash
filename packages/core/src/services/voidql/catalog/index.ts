 ...
 * namespace. A `FROM`/`JOIN` relation must resolve to one of these (or a CTE /
 * derived alias) or the compiler hard-fails (TRQL's `validateTable`-throws model,
 * §11) — so table functions and `system.*` are structurally unreachable.
 */
 ...
 * exposed surface so `analytics_saved_query.schema_version` stays meaningful (§15).
 */
