/**
 * The `events` logical view (§9). Lowers to the audited
 * `resolvedEventsFrom` + `RESOLVED_EVENTS_JOIN` shape — auto-deduped
 ...
 * reachable. The dedup/identity SQL is imported verbatim from
 * `clickhouse-accessor.ts` (single source of truth); VoidQL differs in *one*
 * way: the tenant predicate is **inlined as bound literals into the inner WHERE**
 * (not applied via the `tenantSettings` row-policy setting — the P1 blocker, §20).
 */
import {
...
} from "../../analytics/clickhouse-accessor.ts";
...
const eventOuterProjection = (extra: string): string =>
...
/**
 * The identity-resolution LEFT JOIN, **scoped inline** (§9, §20). The audited
 * `RESOLVED_EVENTS_JOIN` from `clickhouse-accessor.ts` filters the pending-overrides
 * read only by `project_id` because it relied on the readonly user's row policy to
 * supply the `organization_id` predicate. VoidQL runs under the policy-less
 * `analytics_query` user, so we inline `organization_id = {pOrg} AND project_id IN
 ...
 * expressions resolve.
 */
const scopedIdentityJoin = (scope: AuthorizedScope): SqlPiece[] => [
  lit(
