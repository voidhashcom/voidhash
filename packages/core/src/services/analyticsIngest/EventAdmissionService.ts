 ...
 * (`domain/analytics/EventAdmission.ts`); the database only ever holds explicit
...
}): Effect.Effect<never, EventAdmissionServiceError> =>
  Effect.fail(new EventAdmissionServiceError({ message: String(error.cause) }));

const makeEventAdmissionService = (edition: AnalyticsEdition) =>
  Effect.gen(function* () {
    const db = yield* Db;
    ...
    const loadPolicy = (projectId: string) =>
      Effect.gen(function* () {
        ...
        } satisfies EventAdmissionPolicy;
      });

    const resolve = (policy: EventAdmissionPolicy): ResolvedEventAdmissionPolicy => ({
     ...
     */
    const savePolicy = (projectId: string, policy: EventAdmissionPolicy) =>
      db
...
export class EventAdmissionService extends Context.Service<EventAdmissionService>()(
   ...
   */
  static layer = (edition: AnalyticsEdition) =>
