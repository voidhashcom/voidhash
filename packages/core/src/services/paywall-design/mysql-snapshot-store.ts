...
export interface MySqlSnapshotStore {
  ...
  ) => Effect.Effect<SaveDesignStateResult, MySqlSnapshotError | VersionConflictError>;
  ...
  const store: MySqlSnapshotStore = {
    ...
    save: (input: SaveDesignStateInput) =>
      Effect.gen(function* () {
        ...
        if (!isInsert && existing && existing.version !== input.expectedVersion) {
 ...
 */
export const layer: Layer.Layer<MySqlSnapshotStoreTag, never, Db> = Layer.effect(
  MySqlSnapshotStoreTag,
  makeMySqlSnapshotStore
);
