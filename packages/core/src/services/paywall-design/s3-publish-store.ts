...
// 		// Marker class for retryable duplicate key errors
// 		class DuplicateKeyError extends Data.TaggedError("DuplicateKeyError")<{
...
// 		// Helper to check if error is a duplicate key constraint
...
// 			return (
// 				message.includes("Duplicate entry") || message.includes("ER_DUP_ENTRY")
// 			);
// 		};
...
// 				Effect.gen(function* () {
// 					// Get next version number (re-fetched on each retry attempt)
// 					const latestVersion = yield* getLatestVersionQuery(
// 						input.paywallId,
// 					).pipe(
...
// 					// Include versionId in S3 key to prevent concurrent uploads from overwriting each other
// 					const s3Key = `${input.projectId}/${input.paywallId}/v${newVersion}-${versionId}.json`;
...
// 					// Insert version record (may fail on duplicate key if concurrent publish)
// 					// Errors bubble up: duplicate keys handled by retry logic, others by outer catchAll
...
// 					};
// 				}).pipe(
// 					// Retry up to 3 times on duplicate key (concurrent publish race condition)
// 					Effect.retry({
// 						times: 3,
// 						while: (error) => isDuplicateKeyError(error),
// 					}),
// 					// Wrap any remaining duplicate key error as S3PublishError
// 					Effect.catchAll((error) =>
// 						isDuplicateKeyError(error)
// 							? Effect.fail(
// 									new S3PublishError({
// 										cause: new Error(
// 											"Version conflict after retries - concurrent publish detected",
// 										),
// 										operation: "upload",
// 										paywallId: input.paywallId,
// 									}),
