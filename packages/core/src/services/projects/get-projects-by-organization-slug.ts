...
export const getProjectsByOrganizationSlug = Effect.gen(
  function* getProjectsByOrganizationSlug() {
    const db = yield* Db;
    return Effect.fn("getProjectsByOrganizationSlug")(
      function* getProjectsByOrganizationSlug(organizationSlug: string) {
        const session = yield* AuthSession;
        const organization =
          yield* _getOrganizationBySlug(db)(organizationSlug);
        if (!organization) {
          return null;
        }
