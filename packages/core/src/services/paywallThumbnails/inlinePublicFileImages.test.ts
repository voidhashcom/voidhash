...
describe("inlinePublicFileImages", () => {
  it("inlines img src and CSS background urls as data URIs", async () => {
    ...
    const result = await Effect.runPromise(inlinePublicFileImages(html, store));
  ...
  it("fetches each distinct url once and leaves unknown keys untouched", async () => {
  ...
  it("strips query fragments when deriving the key and decodes percent-encoding", async () => {
    const { reads, store } = makeStore({
  ...
  it("skips urls continued by an html-escaped query separator", async () => {
    const { reads, store } = makeStore({
  ...
  it("skips images that would push the document over the byte budget", async () => {
    const { store } = makeStore({
      "big.png": { body: new Uint8Array(64).fill(1), contentType: "image/png" },
      "small.png": { body: new Uint8Array([1]), contentType: "image/png" },
    });
    const html =
      `<img src="${BASE_URL}/files/big.png"/>` + `<img src="${BASE_URL}/files/small.png"/>`;

    const result = await Effect.runPromise(
      inlinePublicFileImages(html, store, { maxHtmlBytes: html.length + 20 }),
    );
