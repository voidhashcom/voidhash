...
import { buildAppStoreSdkContext, type AppStoreSdkContext } from "./sdk-context.ts";
...
const make = Effect.gen(function* () {
  ...
  const buildSdkContextFromConfiguration = Effect.fn("buildSdkContextFromConfiguration")(function* (
    ...
    return buildAppStoreSdkContext(appStoreServerSdk, { ...parsed, inAppPurchasePrivateKey });
  });
        ...
        if (Option.isSome(existingExternal)) {
          if (existingExternal.value.personId !== sdkPersonId) {
            ...
            if (previousIsWebhookStandIn) {
               ...
               * SDK confirmation now reveals the real person — merge the
               * stand-in into it via `identifyDistinctId` (battle-tested merge
               * that archives the source, migrates traits, fans out projection
               * events), then rebind the external identifier.
               */
