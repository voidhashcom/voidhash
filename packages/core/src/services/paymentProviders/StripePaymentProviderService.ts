...
import type { StripePaymentProviderServiceError } from "./stripe/errors.ts";
...
/**
 * Stub service tag for the Stripe payment-provider boundary. The application
 * root provides a `Layer.effect` adapter (`StripePaymentProviderServiceLive`)
 * that delegates to the {@link StripeWebhookHandlerService}.
 */
