...
const makeClient = ClickhouseWebClient.makeUnchecked(() => ({
  url: "http://localhost:8123",
})).pipe(Effect.provide(Reactivity.layer));

describe("VoidQL IR ↔ ch substrate", () => {
  it.effect("toStatement().compile() reproduces the IR's SQL text and binds exactly", () =>
    Effect.gen(function* () {
      const ch = yield* makeClient;
      const cases = [
