	...
	"relative w-full rounded-xl border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
	{
		variants: {
			variant: {
				...
				success:
					"text-foreground bg-success/10 [&>svg]:text-current *:data-[slot=alert-description]:text-muted-foreground border-success dark:border-success/60",
				destructive:
					"text-foreground bg-destructive/10 [&>svg]:text-current *:data-[slot=alert-description]:text-muted-foreground border-destructive",
			},
		},
		defaultVariants: {
			variant: "default",
