...
const redisFactory = (options: RedisOptions) =>
	Effect.gen(function* createRedisClient() {
		...
		const use = <T,>(fn: (client: Redis) => Promise<T>) =>
			Effect.tryPromise({
				catch: (cause) =>
					new RedisError({
						cause,
		...
		const set = (
			...
			Effect.tryPromise({
				...
				try: () => {
					const args: (string | number | Buffer)[] = [key, value];
					if (options?.ex) {
					...
					return redis.set(...(args as [string, string | number | Buffer]));
				},
			});
		...
		const sdiff = (...keys: string[]) =>
			Effect.tryPromise({
				catch: (cause) => new RedisError({ cause, message: "Failed to sdiff" }),
		...
		const zcount = (key: string, min: number | string, max: number | string) =>
			Effect.tryPromise({
				catch: (cause) =>
		...
		const ping = () =>
			Effect.tryPromise({
				catch: (cause) => new RedisError({ cause, message: "Failed to ping" }),
				try: () => redis.ping(),
			});
		...
		const scan = (
			...
			Effect.tryPromise({
				catch: (cause) => new RedisError({ cause, message: "Failed to scan" }),
				try: () => {
...
export type RedisClientType =
	ReturnType<typeof redisFactory> extends Effect.Effect<
		infer T,
		infer _E,
		infer _R
	>
...
export class RedisClient extends Context.Tag("@voidhash/redis/RedisClient")<
	RedisClient,
	RedisClientType
>() {}

export const layer = (options: RedisOptions) =>
	Layer.scoped(RedisClient, redisFactory(options));
