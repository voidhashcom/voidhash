...
const subscriptionIdOption = (purchase: GooglePlayNormalizedPurchase): Option.Option<string> => {
...
const make = Effect.fn("make")(function* () {
   ...
   * replaced an old one.
   *
   * On an upgrade / downgrade / re-signup Play mints a NEW `purchaseToken` and
   * sets `linkedPurchaseToken` to the OLD one; the old token stops granting
   * immediately and never emits its own `SUBSCRIPTION_EXPIRED`. Without this,
   * the two series stay unrelated rows and the old one keeps its perk grant.
   *
   * Expire (not revoke) is the matching action: entitlement ends, but nothing
   * was refunded. The old series is addressed by its own product binding — an
   * upgrade lands on a different base plan, so the *new* purchase's product id
   * would not find it. A missing/unknown/foreign-configuration old series is a
   * silent no-op. Guarded to the first-contact purchase path: the field stays
   * on the subscription resource for the life of the new series, so dispatching
   * it from renewals would re-expire on every RTDN.
   */
  const _expireLinkedSubscriptionSeries = Effect.fn("_expireLinkedSubscriptionSeries")(
    function* (ctx: {
      readonly base: typeof PurchaseActionContext.Type;
      readonly purchase: GooglePlayNormalizedPurchase;
      readonly occurredAt: Date;
    }) {
      if (Option.isNone(ctx.purchase.linkedPurchaseToken)) return;
      const linkedPurchaseToken = ctx.purchase.linkedPurchaseToken.value;
      if (linkedPurchaseToken === ctx.purchase.purchaseToken) return;

      const linkedSubscription = yield* queries.findSubscriptionByStoreSubscriptionId({
        storeSubscriptionId: linkedPurchaseToken,
      });
      if (Option.isNone(linkedSubscription)) return;
      if (
        linkedSubscription.value.paymentProviderConfigurationId !==
        ctx.base.paymentProviderConfigurationId
      ) {
        return;
      }

      yield* Effect.annotateCurrentSpan("google_play.linked_purchase_token", linkedPurchaseToken);
      yield* purchaseProcessingService.expireSubscription({
        ...ctx.base,
        expiredAt: ctx.occurredAt,
        // Redeliveries of the replacing purchase must dedupe onto this one row.
        idempotencyKey: `google:${ctx.purchase.purchaseToken}:linked:${linkedPurchaseToken}`,
        paymentProviderConfigurationProductId:
          linkedSubscription.value.paymentProviderConfigurationProductId,
        providerEventType: "expired",
        providerSubscriptionId: Option.some(linkedPurchaseToken),
        providerTransactionId: Option.some(linkedPurchaseToken),
      });
    },
  );
   ...
   *
   * A subscription that carries a `linkedPurchaseToken` first ends the series
   * it replaced (see {@link _expireLinkedSubscriptionSeries}) so an upgrade
   * cannot leave two live series granting the same perk.
   */
  const recordPurchase = Effect.fn("recordPurchase")(function* (input: GooglePlayRecordInput) {
    ...
    if (isSubscription) {
      yield* _expireLinkedSubscriptionSeries(ctx);
...
export class GooglePlayPaymentProvider extends Context.Service<GooglePlayPaymentProvider>()(
  "@voidhash/backend/purchases/GooglePlayPaymentProvider",
  { make },
) {
