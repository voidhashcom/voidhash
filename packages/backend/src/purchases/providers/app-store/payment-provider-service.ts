  ...
  Effect.gen(function* () {
    ...
    const processSdkTransaction = Effect.fn("processSdkTransaction")(
      function* (input: {
        ...
        // A webhook-first delivery can replay a ledger result recorded against
        // the stand-in that the SDK path just merged. Return its canonical
        // survivor so the response agrees with the now-rebound identity rows.
        const recordedPersonId = yield* queries.resolveCanonicalPersonId({
        ...
        return { parked: false, personId: recordedPersonId } satisfies AppStoreSdkTransactionResult;
      },
