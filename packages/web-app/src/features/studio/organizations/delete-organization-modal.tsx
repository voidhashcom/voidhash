...
export function DeleteOrganizationModal({
  ...
  const deleteOrganizationSchema = z.object({
    confirmation: z.string().refine((value) => value === `${organizationSlug}`, {
      message: "Please enter the text exactly as it is shown to confirm deletion",
    }),
  });
          ...
          <form className="mt-2 space-y-4" onSubmit={form.handleSubmit(onSubmit)}>
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    Please type <span className="select-text font-mono">{organizationSlug}</span> to
                    confirm
                  </FormLabel>
                  <FormControl>
