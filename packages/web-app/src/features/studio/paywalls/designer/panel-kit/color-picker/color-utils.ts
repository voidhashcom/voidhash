...
/**
 * Linearly interpolates between two RGBA color strings at fraction `t` (clamped
 * to `[0, 1]`), returning a new `rgba(...)` string. Used to derive the color of
 * a gradient stop inserted between two neighbors.
 */
export function interpolateRgba(from: string, to: string, t: number): string {
  ...
  return rgbaToString(
    ...
    mix(a.g, b.g),
    mix(a.b, b.b),
    Math.round(mix(a.a, b.a) * 1000) / 1000,
  );
}
