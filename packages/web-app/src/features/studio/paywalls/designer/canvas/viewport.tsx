...
const INTERACTION_END_DEBOUNCE_MS = 120;
// Base snap step at DPR=1; actual step is derived by dividing by current DPR.
const TRANSFORM_SNAP_STEP = 1 / 512;
...
type ZoomRenderMode = "interactive-transform" | "committed-zoom" | "committed-transform";
...
export function Viewport({ children, onTransformChange }: ViewportProps) {
  ...
  const supportsCssZoomRef = useRef<boolean>(
    typeof CSS !== "undefined" && typeof CSS.supports === "function" && CSS.supports("zoom", "1.1"),
  );
  const isInteractingRef = useRef(false);
  const interactionEndTimeoutRef = useRef<number | null>(null);
  const renderModeRef = useRef<ZoomRenderMode>("interactive-transform");
  const committedTransformScaleRef = useRef<number | null>(null);
  ...
  const getTransformSnapStep = useCallback(() => {
    const dpr = Math.max(window.devicePixelRatio || 1, 1);
    return TRANSFORM_SNAP_STEP / dpr;
  }, []);

  const snapScale = useCallback(
    (scale: number) => {
      const step = getTransformSnapStep();
      const snappedScale = Math.round(scale / step) * step;
      return Math.min(Math.max(snappedScale, CANVAS_DEFAULTS.MIN_ZOOM), CANVAS_DEFAULTS.MAX_ZOOM);
    },
    [getTransformSnapStep],
  );

  const clearInteractionEndTimeout = useCallback(() => {
  ...
  const applyTransform = useCallback(() => {
    ...
    if (renderModeRef.current === "committed-zoom") {
      zoomLayer.style.setProperty("zoom", `${scale}`);
      zoomLayer.style.transform = "matrix(1, 0, 0, 1, 0, 0)";
    } else {
      zoomLayer.style.removeProperty("zoom");
      const visualScale =
        renderModeRef.current === "committed-transform"
          ? (committedTransformScaleRef.current ?? scale)
          : scale;
      zoomLayer.style.transform = `matrix(${visualScale}, 0, 0, ${visualScale}, 0, 0)`;
    }

    onTransformChange?.({ scale: transformRef.current.scale, x, y });
  }, [onTransformChange]);

  const commitSharpRender = useCallback(() => {
    if (supportsCssZoomRef.current) {
      renderModeRef.current = "committed-zoom";
      committedTransformScaleRef.current = null;
      applyTransform();
      return;
    }

    const snappedScale = snapScale(transformRef.current.scale);
    transformRef.current.scale = snappedScale;
    committedTransformScaleRef.current = snappedScale;
    renderModeRef.current = "committed-transform";
    applyTransform();
  }, [applyTransform, snapScale]);
  ...
  const beginInteraction = useCallback(() => {
    ...
    if (renderModeRef.current !== "interactive-transform") {
      renderModeRef.current = "interactive-transform";
      committedTransformScaleRef.current = null;
      applyTransform();
    }
  }, [applyTransform, clearInteractionEndTimeout]);
  ...
  const endInteraction = useCallback(() => {
    ...
    if (zoomLayer) {
      zoomLayer.style.willChange = "";
    }

    commitSharpRender();
  }, [clearInteractionEndTimeout, commitSharpRender]);
  ...
  }, [clearInteractionEndTimeout, endInteraction]);
  ...
  const initializePosition = useCallback(() => {
    ...
    transformRef.current.x =
      (containerRect.width - scaledScreenWidth) / 2 - INIT_SCREEN_DATA.x * scale;
    transformRef.current.y =
      (containerRect.height - scaledScreenHeight) / 2 - INIT_SCREEN_DATA.y * scale;

    applyTransform();
    commitSharpRender();
  }, [applyTransform, commitSharpRender]);
  ...
  }, [beginInteraction, pan, scheduleInteractionEnd, zoomAtPoint]);
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      const { initialDistance, initialScale, lastMidpoint } = touchStateRef.current;
