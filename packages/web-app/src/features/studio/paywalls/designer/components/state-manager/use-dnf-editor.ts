...
export function useDNFEditor() {
    ...
    (conjunctionIndex: number, variables: readonly CRDTVariable[]) => {
      setEditingDNF((prev) => {
        ...
        newValue[conjunctionIndex] = {
          type: "and" as const,
          value: [...existingConjunction.value, createDefaultPredicate(variables)],
        };
  ...
  const removePredicate = useCallback((conjunctionIndex: number, predicateIndex: number) => {
    setEditingDNF((prev) => {
      if (!prev) {
        return prev;
      }
  ...
  const updatePredicate = useCallback(
    (conjunctionIndex: number, predicateIndex: number, predicate: LocalPredicate) => {
      setEditingDNF((prev) => {
        if (!prev) {
          return prev;
        }
  ...
  const getDNFForSave = useCallback((): DnfInput | null => {
    if (!editingDNF) {
      return null;
    }
    return localToDnfInput(editingDNF);
  }, [editingDNF]);
