...
export function ProductDetailProviderProductRecord({
  ...
  const paymentProvider = paymentProviders.find((p) => p.id === paymentProviderId);

  const [openEditSheet, setOpenEditSheet] = useState(false);

  const { mutate: deleteProviderProduct, status: deleteProviderProductStatus } = useMutation({
    ...deletePaymentProviderProductOptions(),
    onSuccess: () => {
  ...
  const { mutate: setActiveProviderProduct, status: setActiveProviderProductStatus } = useMutation({
    ...setActivePaymentProviderProductOptions(),
    onSuccess: () => {
  ...
  const isSettingActiveProviderProduct = setActiveProviderProductStatus === "pending";
  ...
  const handleDeleteProviderProduct = async () => {
    ...
    const res = await openDialog({
      description: `Are you sure you want to delete this ${paymentProvider.title} product? This may break access for people who have already purchased this.`,
  ...
  return (
    ...
    >
      <div className={cn("flex flex-row gap-2", !providerProduct.isActive && "opacity-50")}>
        {paymentProvider.productConfigurationKeyProperties.map((key) => {
          const value = providerProduct.configuration?.[key];
      ...
      <div className="flex flex-row gap-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48">
            <DropdownMenuItem onSelect={() => setOpenEditSheet(true)}>Edit</DropdownMenuItem>
            <DropdownMenuItem
            ...
            <DropdownMenuItem disabled={isPending} onSelect={handleDeleteProviderProduct}>
