...
binary on `PATH`. See the [CLI reference](/cli).
...
[Development purchase mode](/development-mode) for fixed prices, isolation, and lifecycle tools.
...
[Display a paywall](/paywalls) for every status.
