---
title: CLI
description: Connect projects, generate types, preview paywalls, and deploy from the terminal.
icon: SquareTerminal
---
...
| `studio`                      | Preview code-authored paywalls locally.                                          |
| `deploy`                      | Build and upload code-authored paywalls.                                         |
...
## Preview and deploy code paywalls

```bash
npx voidhash-cli studio
```

Studio reads paywalls and components from the project's `.voidhash` directory. When the preview is
ready, validate without uploading:
...
- **No paywalls found during deploy** — add a paywall under `.voidhash/paywalls`, or use Studio's
  visual paywall workflow instead.
