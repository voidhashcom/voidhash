...
| `FAILED_TO_PURCHASE`            | `purchase(...)`              | See [Purchases](/sdks/kotlin/purchases).                       |
| `FAILED_TO_RESTORE_PURCHASES`   | `restorePurchases()`         | Prompt the customer to retry.                                  |
| `FAILED_TO_PRESENT_PAYWALL`     | `presentPaywall(...)`        | Reported as a `false` return; fall back to your own screen.    |
