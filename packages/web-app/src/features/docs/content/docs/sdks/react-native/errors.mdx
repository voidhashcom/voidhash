...
| `FAILED_TO_PURCHASE`                        | `purchase()`                     | Delivered as `{ status: "failed", error }`; see [Purchases](/sdks/react-native/purchases).     |
...
| `READ_ONLY_PURCHASE_NOT_ALLOWED`   | Observer mode blocks writes. See [Migrate in observer mode](/resources/observer-mode). |
...
[`Result`](/sdks/react-native/purchases#start-a-purchase), and only the `Err` channel carries an error — typically
`FAILED_TO_PURCHASE`.
