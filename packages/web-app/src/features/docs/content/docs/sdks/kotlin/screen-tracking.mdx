...
not reported. Closing the handle detaches the listener and lets activity screens through again.
...
Each resumed fragment is recorded with its class name. Framework fragments such as
`NavHostFragment` are skipped. Activities hosting tracked fragments are not reported themselves.
...
arguments often carry ids. `mapScreen` lets you rename a screen or drop it by returning `null`.
