...
This calls `AppStore.sync()` and reads verified non-consumable transaction history, including
expired or revoked transactions. App Store authentication may appear, so call it only from a
user action.
