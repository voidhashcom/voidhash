...
[server-side entitlement checks](/libraries/node/check-access) as the authority. Webhooks are a
...
so you must verify the raw request body before anything parses it.

This Express handler keeps the raw body, verifies the signature, and acknowledges the delivery
