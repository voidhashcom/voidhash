...
[server-side entitlement checks](/libraries/go/check-access) as the authority. Webhooks are a
...
You can also create the same endpoint from your backend. This program creates an endpoint that
subscribes to three events and prints its signing secret:

```go title="scripts/create-webhook.go"
endpoint, err := voidhash.Webhooks.CreateWebhookEndpoint(ctx, &voidhash.CreateWebhookEndpointParams{
...
fmt.Println(endpoint.Secret) // whsec_...
```
...
so read the body with `io.ReadAll(r.Body)` and verify it before anything decodes it. A framework
that re-serializes the body breaks verification.

This `net/http` handler reads the raw body, verifies the signature, and acknowledges the delivery
before doing any work:

```go title="server/webhooks.go"
import "github.com/voidhash/voidhash-go/webhook"

func voidhashWebhookHandler(w http.ResponseWriter, r *http.Request) {
    ...
    event, err := webhook.ConstructEvent(rawBody, r.Header, os.Getenv("VOIDHASH_WEBHOOK_SECRET"))
    if err != nil {
        // errors.Is(err, webhook.ErrMissingHeader)
        //        | webhook.ErrInvalidSignature
        //        | webhook.ErrTimestampOutOfRange | webhook.ErrInvalidPayload
        w.WriteHeader(http.StatusBadRequest)
        return
    }
...
`ConstructEvent` returns a `webhook.Event` with `Type string`, `Payload json.RawMessage`, and
`Timestamp int64`. Event names added after your library release pass through as plain strings in
`Type`, so always give your switch on the type a default case. When you need a typed payload,
decode it on demand with `json.Unmarshal(event.Payload, &renewed)`.
...
infrastructure adds delay, raise the limit by passing options with a larger `ToleranceSeconds`. Set
`Now` in the same options to make tests deterministic.

This call allows a 600 second window:

```go
event, err := webhook.ConstructEventWithOptions(rawBody, r.Header, secret, webhook.VerifyOptions{
    ToleranceSeconds: 600,
})
```

Keep your server clocks synchronized. Clock drift is the most common cause of
`ErrTimestampOutOfRange` on an integration that is otherwise correct.
...
asynchronously. Hand the event to a worker pool or enqueue it. A handler that does real work before
responding is the usual reason a delivery times out and arrives a second time.

You can see each delivery and its per-attempt history in Studio. The same data is readable through
`ListWebhookDeliveries`, `GetWebhookDelivery`, and `RetryWebhookDelivery` on the webhooks service.
...
```go
voidhash.Webhooks.TestWebhookEndpoint(ctx, "wh_ep_...")
```
...
```go
endpoint, err := voidhash.Webhooks.RotateWebhookSecret(ctx, "wh_ep_...")

fmt.Println(endpoint.Secret) // new whsec_...
```
...
- [Check access from your backend](/libraries/go/check-access)
