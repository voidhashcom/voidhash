...
[server-side entitlement checks](/libraries/go/check-access) as the authority for an access decision:
...
so read the body with `io.ReadAll(r.Body)` before any decoding — a framework that re-serializes the
body breaks verification.
...
`Type`, so always give your switch on the type a default case. Typed payloads decode on demand with
`json.Unmarshal(event.Payload, &renewed)`.
...
direction. Pass options if your infrastructure adds delay, and set `Now` to make tests deterministic:
...
Keep server clocks synchronized. Clock drift is the most common cause of `ErrTimestampOutOfRange`
on an otherwise correct integration.
...
Return `200` as soon as the signature verifies, then process asynchronously — hand the event to a
worker pool or enqueue it. A handler that does real work inline is the usual reason a delivery times
out and is duplicated.
