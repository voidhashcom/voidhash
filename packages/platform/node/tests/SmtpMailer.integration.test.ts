...
const readConfig = Effect.gen(function* () {
  const config: SmtpMailerConfig = {
    host: yield* Config.string("PLATFORM_NODE_SMTP_HOST").pipe(Config.withDefault("127.0.0.1")),
    port: yield* Config.int("PLATFORM_NODE_SMTP_PORT").pipe(Config.withDefault(1025)),
...
const mailerLayer = (adjust: (config: SmtpMailerConfig) => SmtpMailerConfig = (input) => input) =>
  Layer.unwrap(
    readConfig.pipe(
      Effect.map((config) => Layer.merge(SmtpMailerLive(adjust(config)), NodePlatformRuntimeLive)),
    ),
  );

const mailpitApi = Config.string("PLATFORM_NODE_MAILPIT_API").pipe(
  ...
  it("validates message content through the stable error channel", () =>
    Effect.runPromise(
      Effect.gen(function* () {
        ...
        }).pipe(Effect.provide(mailerLayer((config) => ({ ...config, verifyOnStart: false }))));
