...
const makeQueueDriver = (
  ...
  const producer = <A, I>(queueName: string, schema: Schema.Codec<A, I>): QueueProducer<A> => {
...
> = Layer.effect(QueueDriver, Effect.map(PersistedQueue.PersistedQueueFactory, makeQueueDriver));
