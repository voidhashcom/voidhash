...
function nextPathName(pathCounter: { count: number }, fallbackPrefix: string, id: string | null): string {
  pathCounter.count += 1;
...
function lineToPath(element: Element): string {
...
function parseViewBox(viewBoxString: string | null): ViewBox {
  ...
  const parts = viewBoxString.trim().split(/[\s,]+/).map(Number);
  if (parts.length !== 4) {
    return { ...FALLBACK_VIEWBOX };
  }
...
function buildRootInheritedStyles(svg: Element): Record<string, string> {
