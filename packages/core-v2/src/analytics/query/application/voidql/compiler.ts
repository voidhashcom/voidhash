 ...
 * **logical-view substitution** so each base relation lowers to a
 ...
 * identifiers. The result feeds the value-level verifier and {@link toStatement}.
 *
...
/** Largest result set VoidQL will return; the server clamps every LIMIT to it. */
...
const orderDirectionSql = (dir: OrderItem["dir"]) => {
  if (dir === "desc") return " DESC";
  return " ASC";
};

/** `NULLS FIRST`/`NULLS LAST` for an ORDER BY item. */
const orderNullsSql = (nulls: "first" | "last") => {
  if (nulls === "first") return " NULLS FIRST";
  return " NULLS LAST";
};

const joinKindSql = (kind: Join["kind"]) => {
...
export class Compiler {
   ...
   * rather than an upstream invariant. Every alias/column/namespace identifier
  ...
  /** Lower a JSON-namespace property access; the key is bound, never spliced. */
