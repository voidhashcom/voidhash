 ...
 * (NOT backslash) escaping a quote. A dialect escape mismatch is an isolation
 * risk, so comments are dropped before the canonical re-print.
 *
...
export const lex = (text: string): readonly Token[] => {
  ...
  const peek = (ahead = 0) => text[offset + ahead] ?? "";
  ...
  const fail = (start: Pos, message: string) =>
    raise(
