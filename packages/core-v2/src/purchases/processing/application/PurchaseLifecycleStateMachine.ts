...
const makePurchaseLifecycleStateMachine = Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        const outcome = yield* unitOfWork.transact(
          Effect.gen(function* () {
            ...
            if (claim._tag === "duplicate") return { events: [], result: claim.result };
  ...
  const mutateKnownSubscription = <I extends Action>(input: {
    ...
    readonly update: (
      existing: import("../../application/ports.ts").PurchaseSubscriptionRecord,
    ) => Omit<import("../../application/ports.ts").PurchaseSubscriptionUpdate, "id" | "occurredAt">;
  }) =>
    mapErrors(
      Effect.gen(function* () {
  ...
  const cancelSubscription = (input: typeof CancelSubscriptionInput.Type) =>
    mutateKnownSubscription({
      ...
      buildWebhook: (existing, updated, context) => {
        ...
        if (!input.cancelAtPeriodEnd) status = subscriptionStatusForInactiveEvent();
        return () =>
          toSubscriptionCancelledWebhookEvent(
            input,
      ...
      Effect.gen(function* () {
        ...
        return yield* unitOfWork.transact(
          Effect.gen(function* () {
            ...
            let transaction: PurchaseTransactionRecord | undefined;
