 ...
 * All mutations go through Editor commands with automatic index management.
 */
 ...
 * Uses editor.commands.createNode for automatic index management.
 *
 * @example
 * ```ts
...
export function createNodeAction<TNodeType extends CreatableNodeType>(
  ...
  const action = storeState.action(
    ({ params, getState, doc, dispatch }: any) => {
      ...
      // Create node using editor commands (handles ID and index generation)
      const editor = createEditorForDoc(doc);
      const handle = editor.commands.createNode(nodeType, {
        parentId: typedParams.parentId,
        beforeSiblingId: typedParams.beforeSiblingId ?? null,
        data: typedParams.initialValues as any
      });
 ...
 * All mutations go through Editor with YjsStorage for validation.
 *
 * @example
 * ```ts
