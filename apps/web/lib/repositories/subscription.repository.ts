  ...
  'SubscriptionRepository',
  {
    effect: Effect.gen(function* () {
      ...
      return {
        ...
        getSubscriptionById: dbService.makeQuery((execute, id: string) =>
          execute(
            async (db) =>
        ...
        getSubscriptionsByCustomerIdWithPaymentProviderConfigurationProduct:
        ...
        updateSubscription: dbService.makeQuery(
          (
            execute,
            input: Omit<Partial<Subscription>, 'id'> & { id: string }
          ) =>
            execute(
              async (db) =>
                await db
