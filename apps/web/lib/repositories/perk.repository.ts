...
import { and, eq, inArray, InsertPerk, paymentProviderConfigurationProducts, perks, productPerks, products } from "@voidhash/db";
	...
	"PerkRepository",
	{
		effect: Effect.gen(function* () {
			...
			return {
				...
				getPerksByPaymentProviderConfigurationProductIds: dbService.makeQuery((execute, paymentProviderConfigurationProductIds: string[]) =>
					execute(async (db) =>
						(await db.select().from(paymentProviderConfigurationProducts)
							.innerJoin(products, eq(paymentProviderConfigurationProducts.productId, products.id))
							.innerJoin(productPerks, eq(products.id, productPerks.productId))
							.innerJoin(perks, eq(productPerks.perkId, perks.id))
							.where(inArray(paymentProviderConfigurationProducts.id, paymentProviderConfigurationProductIds))).map((row) => row.perk)
					)
				),
