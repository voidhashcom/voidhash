...
import { Customer, CustomerOrigin, CustomerType, InsertCustomer, PaywallProduct } from "@voidhash/db";
				...
				createCheckout: (input: {
					...
					pipe(
								...
								TransactionContext.provide(tx)(
									Effect.gen(function* () {
										...
										if (!customer && isAnonymousId(appUserId)) {
											const newCustomer = yield* customerService.createAnonymousCustomer({
												projectId,
												appUserId,
						...
						Environment.withEnvironment({
							projectId: input.paymentProviderConfigurationProductId, // TODO: Get from product
						}),
