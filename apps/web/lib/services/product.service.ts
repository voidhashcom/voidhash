				...
				updatePaymentProviderProduct: (input: {
					...
					Effect.gen(function* () {
						...
						const [product, providerConfiguration] = yield* Effect.all([
							...
							paymentProviderConfigurationRepository.getPaymentProviderConfigurationById(providerProduct.paymentProviderConfigurationId)
						]);
						...
						const provider = paymentProviders.find(
							(p) =>
								p.getId() === providerConfiguration.providerId
						);
						if (!provider) {
							return yield* Effect.fail(
								new PaymentProviderNotFound({
