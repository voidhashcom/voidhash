...
import { and, eq, isNull, ne, InsertPaymentProviderConfiguration, paymentProviderConfigurations } from "@voidhash/db";
import { Effect } from "effect";

export class PaymentProviderRepository extends Effect.Service<PaymentProviderRepository>()(
	"PaymentProviderRepository",
	{
		effect: Effect.gen(function* () {
			...
			return {
				...
				getExistingPaymentProviderConfigurationByProviderId: dbService.makeQuery(
					(
						execute,
						input: {
							projectId: string;
							...
							async (db) =>
								await db
									...
									.set({
										...
										...(input.paymentProviderKey !== undefined && { paymentProviderKey: input.paymentProviderKey }),
									})
