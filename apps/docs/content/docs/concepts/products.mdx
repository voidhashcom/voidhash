...
<Callout type="info" title="Base Plan ID">
  The `basePlanId` is only needed for Google Play subscriptions that use base
  plans. This is a Google Play feature that allows you to offer different
  billing periods and pricing for the same product.
</Callout>
