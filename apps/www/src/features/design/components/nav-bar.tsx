...
function SearchToggle(props: ComponentProps<"button">) {
  ...
  return (
    <button
      {...props}
      className={cn(
        "flex min-w-[200px] items-center gap-2 bg-transparent text-sm md:bg-muted",
        props.className,
      )}
      onClick={() => setOpenSearch(true)}
    >
      <SearchIcon className="size-4 cursor-pointer text-muted-foreground " />
      <span className="mr-4 hidden flex-1 text-left text-muted-foreground md:block">Search...</span>
