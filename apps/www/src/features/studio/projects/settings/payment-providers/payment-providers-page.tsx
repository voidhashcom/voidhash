...
export const PaymentProvidersPage = () => {
  ...
  const applicationsWithConfiguration = (paymentProviderConfigurations ?? [])
    .map((p) => {
      const paymentProvider = paymentProviders.find((pp) => pp.id === p.providerId);
    ...
    .filter(Boolean);
  const configuredStoreProviderIds = applicationsWithConfiguration.flatMap((configuration) =>
  ...
  return (
    <Page>
      ...
      <div className="mx-auto max-w-4xl">
        <h1 className="font-normal text-3xl tracking-right">Payment Providers</h1>
        <p className="mt-3 text-muted-foreground">Configure your payment providers.</p>
        ...
        <div className="mt-8">
          <Card className={cn("grid gap-0 divide-y p-0")}>
              ...
              <div className="flex items-center justify-between">
                ...
                {applicationsWithConfiguration.length > 0 && (
                  <PaymentProvidersNewStoreDropdown
                    configuredProviderIds={configuredStoreProviderIds}
            ...
            {applicationsWithConfiguration?.map((paymentProviderConfiguration) =>
