...
export function ProductRecordConfigurationStateIndicator({
  ...
  if (providerProductsStatus === "error" || paymentProviderConfigurationsStatus === "error") {
    return <Badge>Loading error</Badge>;
  }

  if (providerProductsStatus === "pending" || paymentProviderConfigurationsStatus === "pending") {
  ...
  if (!paymentProviderConfigurations || paymentProviderConfigurations.length === 0) {
    return <Badge>Configuration required</Badge>;
  }
      ...
      {paymentProviderConfigurations
        ...
        .map((paymentProviderConfiguration) =>
          providerProducts.some(
            (providerProduct) =>
              providerProduct.paymentProviderConfigurationId === paymentProviderConfiguration.id,
          ) ? (
            <PaymentProviderLogo
              className="h-4 w-4"
              key={paymentProviderConfiguration.providerId}
              providerId={
                paymentProviderConfiguration.providerId as (typeof paymentProviders)[number]["id"]
              }
            />
          ) : null,
        )}
