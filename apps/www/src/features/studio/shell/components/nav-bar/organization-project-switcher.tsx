...
function SwitcherPopoverTrigger() {
  return (
    <PopoverTrigger asChild>
      <Button
        className="px-1 focus-visible:ring-0 data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground   cursor-pointer"
        size="icon"
        variant="ghost"
      >
        <ChevronsUpDown />
    ...
    <div className="w-56 bg-accent/30">
      ...
      {(projects ?? []).map((project) => (
        ...
        >
          ...
          />
          {project.name}
          {project.id === activeProjectId && <Check className="ml-auto h-4 w-4" />}
        </Link>
      ))}
...
export function OrganizationProjectSwitcher({
  ...
  const [highlightedOrganizationIndex, setHighlightedOrganizationIndex] = React.useState<
    number | null
  >(null);
  const highlightedOrganization =
    highlightedOrganizationIndex !== null ? organizations[highlightedOrganizationIndex] : null;

  const [createOrganizationModalOpen, setCreateOrganizationModalOpen] = React.useState(false);

  return (
    <Popover onOpenChange={setOpen} open={open}>
      <SwitcherPopoverTrigger />
      <PopoverContent
        align="end"
      ...
      >
        {activeOrganization && (
          ...
          >
            <div className="w-64">
              <div className="px-2 py-1.5 text-muted-foreground text-xs">Teams</div>
              {organizations.map((organization, index) => (
                <Link
                  className={cn(
                    "flex w-full items-center gap-2 p-2 text-foreground text-sm hover:bg-accent/50 hover:text-accent-foreground",
                    organization.slug ===
                      (highlightedOrganization?.slug ?? activeOrganization?.slug) && "bg-accent/50",
                  )}
            ...
            <OrganizationProjectSwitcherProjects
              ...
              organizationId={highlightedOrganization?.id ?? activeOrganization.id}
              organizationSlug={highlightedOrganization?.slug ?? activeOrganization.slug}
              projects={user.projects.filter(
                (p) => p.organizationId === (highlightedOrganization?.id ?? activeOrganization.id),
              )}
            />
          </div>
        )}
...
export function ProjectsPicker({
  ...
  user: { projects: UserType["projects"] };
  activeProject: UserType["projects"][number] | null;
  activeOrganization: UserType["organizations"][number];
}) {
  const [open, setOpen] = React.useState(false);
  const projects = user.projects.filter((p) => p.organizationId === activeOrganization.id);
