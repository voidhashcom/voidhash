...
export function PanelSectionHeader({
  ...
  return (
    <div className={cn("my-2 flex h-7 flex-row items-center justify-between px-4", className)}>
      {children}
    </div>
  );
}
...
export function PanelSectionHeaderActions({
  ...
  return (
    <div className={cn("flex flex-row items-center justify-end gap-2", className)}>{children}</div>
  );
}
...
export function PanelSubSectionTitle({
  ...
  return (
    <div className={cn("font-medium text-muted-foreground text-xs", className)}>{children}</div>
  );
}
