...
import { BUILTIN_COMPONENT_ICON, type NodeIconKey, NODE_TYPE_ICONS } from "../node-type-icons";
import {
  moveNode,
  selectNode,
  setHighlightedNode,
...
import { nodePassesSlotGate } from "../../state/utils/component-children";
 ...
 * layers tree reads. Snapshot nodes nest their payload under `data`; `style`
 * stays untyped because each node type carries a different style struct.
 */
...
function flattenTree(
  ...
  if (node.type === "root") {
    for (const child of node.children ?? []) {
      flattenTree(child, expandedLayers, canAcceptChildren, depth, node.id, result);
    }
    return result;
  }

  const canHaveChildren =
    nodeCanHaveChildren(node) && node.type !== "root" && canAcceptChildren(node);
  ...
  if (isExpanded) {
    for (const child of node.children ?? []) {
      flattenTree(child, expandedLayers, canAcceptChildren, depth + 1, node.id, result);
    }
  }
...
function removeChildrenOf(items: FlattenedNode[], ids: string[]): FlattenedNode[] {
...
function getProjection(
  ...
  if (parentId === "root") {
    ...
    if (
      !(canBeChildOf(activeNodeType, parentItem.node.type) && canAcceptChildren(parentItem.node))
    ) {
      ...
      while (currentParentId) {
        ...
        if (
          canBeChildOf(activeNodeType, ancestorItem.node.type) &&
          canAcceptChildren(ancestorItem.node)
        ) {
...
function getTargetIndex(
  ...
  const siblingIndex = children.findIndex((child) => child.id === beforeSiblingId);
  return siblingIndex !== -1 ? siblingIndex : children.length;
}

function getHighlightedNodeIds(projectedParentId: string | null, tree: SnapshotNode): Set<string> {
...
function SortableTreeItem({
  ...
  // Builtin component instances get the distinct blocks glyph, matching the
  // add-component browser; catalog/local components keep the generic component icon.
  const isBuiltinComponent =
    node.type === "component" &&
    (node.data as { componentSource?: string } | undefined)?.componentSource === "builtin";
  const Icon = isBuiltinComponent
    ? BUILTIN_COMPONENT_ICON
    : NODE_TYPE_ICONS[iconType as NodeIconKey];
  const displayName = node.data?.name ?? "Unknown";
  ...
  const showIndicatorAbove = projected && projected.overId === id && projected.isAbove;
  const showIndicatorBelow = projected && projected.overId === id && !projected.isAbove;

  // Show indicator below if this is the last item and drop is at the end
  const showIndicatorAtEnd = isLast && projected && projected.overId === END_DROP_ZONE_ID;
  ...
  const opacityClass = isDraggingSomething && !isActiveItem && !isHighlighted ? "opacity-40" : "";
  ...
  return (
    <DesignerContextMenu nodeId={id} source="layers-panel">
      <div
      ...
      >
        ...
        <div
          className={cn(
            ...
            isChildOfSelected && !isSelected && "bg-accent/40 text-accent-foreground/80",
          )}
          onClick={handleClick}
        ...
        >
          ...
          <div className="flex flex-1 items-center gap-1.5 overflow-hidden">
            ...
            {isEditing ? (
              <input
                className="h-5 w-full min-w-0 rounded border border-primary bg-background px-1 font-medium text-xs outline-none"
              ...
              <span className="truncate font-medium text-xs select-none">{displayName}</span>
            )}
            {hasWarning && !isEditing && (
              <span
                className="ml-auto flex size-3.5 shrink-0 items-center justify-center [&_svg]:size-3"
...
export function LayersSection() {
  ...
  const componentCatalogByHash = useStore(store, (state) => state.componentCatalog.byContentHash);
  const dispatch = usePaywallDesignerActions();

  const canAcceptChildren = useCallback(
    (node: SnapshotNode) => nodePassesSlotGate(node, componentCatalogByHash),
    [componentCatalogByHash],
  );

  // Combine confirmed and preview selections for display
  const allSelectedNodeIds = useMemo(
    () => [...new Set([...selectedNodeIds, ...previewSelectedNodeIds])],
    [selectedNodeIds, previewSelectedNodeIds],
  );
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (currentOverId && currentOverId !== END_DROP_ZONE_ID) {
        const overElement = document.querySelector(`[data-id="${currentOverId}"]`) as HTMLElement;
        if (overElement) {
  ...
  // The document root is already a tree structure from mimic
  const tree: SnapshotNode = documentRoot;
  ...
  const flattenedItems = useMemo(() => {
    ...
    const flattened = flattenTree(tree, allExpandedLayers, canAcceptChildren);
    ...
    return flattened;
  }, [tree, allExpandedLayers, activeId, canAcceptChildren]);
    ...
    if (overId === END_DROP_ZONE_ID) {
      ...
      if (lastItem && depth > 0) {
        ...
        while (current && current.depth >= depth) {
          ...
          current = flattenedItems.find((item) => item.id === current?.parentId);
        }
        if (!current && depth > 0) {
          parentId = lastItem.canHaveChildren ? lastItem.id : lastItem.parentId;
          depth = lastItem.canHaveChildren ? lastItem.depth + 1 : lastItem.depth;
        }
      }

      // Validate parent-child relationship (catalog rules + component slot gate)
      const parentItem =
        parentId === "root" ? null : flattenedItems.find((item) => item.id === parentId);
      const parentType = parentId === "root" ? "root" : (parentItem?.node.type ?? "root");
      if (!canBeChildOf(activeItem.node.type, parentType)) {
        return null;
      }
      if (parentItem && !canAcceptChildren(parentItem.node)) {
        return null;
      }
  ...
  }, [activeId, overId, offsetLeft, flattenedItems, cursorY, overItemHeight, canAcceptChildren]);
  ...
  const handleFinishEdit = useCallback(() => {
    ...
    if (trimmedName && trimmedName !== originalNameRef.current) {
      const editingNode = flattenedItems.find((item) => item.id === editingNodeId);
      if (editingNode) {
        const nodeType = editingNode.node.type;
        if (nodeType === "view") {
          dispatch(updateViewNode)({
            id: editingNodeId,
  ...
  }, [editingNodeId, editingName, flattenedItems, dispatch]);
  ...
  const handleDragOver = useCallback(
    ({ over }: DragOverEvent) => {
      ...
      if (newOverId && newOverId !== overIdRef.current && newOverId !== END_DROP_ZONE_ID) {
        const overElement = document.querySelector(`[data-id="${newOverId}"]`) as HTMLElement;
        if (overElement) {
          const rect = overElement.getBoundingClientRect();
  ...
  const handleDragEnd = useCallback(
    ({ active, over }: DragEndEvent) => {
      ...
      if (isAbove) {
        ...
        beforeSiblingId = getSiblingAfter(flattenedItems, over.id as string, newParentId);
      }
          ...
          {flattenedItems.length > 0 ? (
            flattenedItems.map((item, index) => (
              <SortableTreeItemWrapper
                ...
                allSelectedNodeIds={allSelectedNodeIds}
            ...
            <div className="px-4 py-2 font-medium text-muted-foreground text-xs">No layers yet</div>
          )}
          {/* End drop zone indicator */}
          {projected && projected.overId === END_DROP_ZONE_ID && flattenedItems.length > 0 && (
            <div
...
function SortableTreeItemWrapper({
  ...
  // Check if selected (either confirmed or preview selection)
  const isSelected = allSelectedNodeIds.includes(item.id);
  const store = usePaywallDesignerStore();
  const dispatch = usePaywallDesignerActions();
  const hoveredNodeId = useStore(
    store,
    useShallow((state) => state.highlightedNodeId ?? null),
  );
  const warnings = useComponentNodeWarnings(item.id);
  const hasWarning = warnings.some((warning) => warning.severity === "warning");

  const isHighlighted = highlightedIds.has(item.id);
  const isChildOfSelected = isDescendantOfSelected(item, flattenedItems, allSelectedNodeIds);
  const isEditing = editingNodeId === item.id;

  const handleStartEdit = useCallback(() => {
    onStartEdit(item.id, item.node.data?.name ?? "");
  }, [item.id, item.node.data?.name, onStartEdit]);

  const handleMouseEnter = () => {
    dispatch(setHighlightedNode)({ id: item.id });
  };

  const handleMouseLeave = () => {
    dispatch(setHighlightedNode)({ id: null });
  };
