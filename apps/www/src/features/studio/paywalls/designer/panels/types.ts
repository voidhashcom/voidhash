...
/**
 * Sentinel symbol indicating that selected nodes have different values for a property.
 */
export const MIXED: unique symbol = Symbol("MIXED");
export type Mixed = typeof MIXED;

/**
 * A value that may be a concrete T or the MIXED sentinel (when multiple nodes disagree).
 */
export type MaybeMulti<T> = T | Mixed;

/**
 * Section IDs for the right panel registry.
 */
...
/**
 * A section registry entry describes a panel section, which node types it supports,
 * and whether it can appear in multi-select mode.
 */
export interface SectionRegistryEntry {
  id: SectionId;
  supportedNodeTypes: EditableNodeType[];
  order: number;
