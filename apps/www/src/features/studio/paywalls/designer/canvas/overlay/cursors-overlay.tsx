...
>(({ x, y, name, color, ...rest }, ref) => (
  <div
    ...
    {...rest}
  >
    <svg width="20" viewBox="0 0 51 59" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export function CursorsOverlay() {
  ...
  const cursors = [...(others?.values() ?? [])].flatMap((value) => {
    const data = value.data;
    if (!data) {
      return [];
    }
    const screenPos = canvasToScreen(data.cursor?.x ?? 0, data.cursor?.y ?? 0);
    return [
      {
  ...
  return (
    <div className="pointer-events-none absolute w-screen h-screen inset-0" style={{ zIndex: 11 }}>
      {cursors.map((cursor) => (
