...
describe("identityAttributes", () => {
  it("stamps the acting user, first org/project, and counts for a user session", () => {
    ...
    expect(attrs.get("voidhash.user.workos_id")).toBe("wos_user_1");
    ...
    expect(attrs.get("voidhash.organization.workos_id")).toBe("wos_org_1");
  ...
  it("omits nullable fields rather than emitting the string null", () => {
    ...
    expect(attrs.has("voidhash.user.workos_id")).toBe(false);
    expect(attrs.has("voidhash.user.role")).toBe(false);
    expect(attrs.has("voidhash.organization.workos_id")).toBe(false);
