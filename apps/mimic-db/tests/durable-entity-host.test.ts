...
describe("memory DurableEntity host", () => {
  test("serializes operations for the same address in submission order", () =>
    Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* Effect.all([operation("first"), operation("second")], {
  ...
  test("allows different entity addresses to run concurrently", () =>
    Effect.runPromise(
      Effect.gen(function* () {
        const host = makeMemoryDurableEntityHost();
        let active = 0;
        ...
        yield* Effect.all([operation("one"), operation("two")], { concurrency: "unbounded" });

        expect(maxActive).toBe(2);
      }),
    ));

  test("retains entity-local key-value, alarm, and session state", () =>
    Effect.runPromise(
      Effect.gen(function* () {
        const host = makeMemoryDurableEntityHost();
        const address = makeDurableEntityAddress("document", "one");
