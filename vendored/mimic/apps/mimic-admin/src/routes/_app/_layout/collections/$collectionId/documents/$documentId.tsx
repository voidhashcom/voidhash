...
function DocumentPage() {
  ...
  const { data: document, isLoading } = useQuery(documentQuery(sdk, collectionId, documentId));
  ...
  const setMutation = useMutation({
    mutationFn: () => {
      ...
      return sdk.database("").collectionRaw(collectionId).setDocumentRaw(documentId, data);
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => sdk.database("").collectionRaw(collectionId).deleteDocument(documentId),
    onSuccess: () => {
      void queryClient.invalidateQueries({
          ...
          {generatedAuth && (
            <div className="space-y-2 rounded-md bg-muted p-3">
              <div className="flex items-center gap-2">
                <code className="flex-1 break-all text-xs">{generatedAuth.url}</code>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => {
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 break-all text-xs">{generatedAuth.token}</code>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => {
